...
class EventProcessor:
    ...
    async def process_event(self, event_data: Dict[str, Any], message_id: str) -> bool:
        ...
        try:
            ...
            event_processing_duration.labels(event_type=event.event_type).observe(
                processing_time
            )
    ...
    async def _move_to_dlq(self, event_data: Dict[str, Any], error_message: str):
        ...
        try:
            ...
            self.logger.info(
                "Event moved to DLQ",
                extra={"event_id": event_data.get("event_id"), "error": error_message},
            )
        except Exception as e:
            self.logger.error(f"Failed to move event to DLQ: {str(e)}", exc_info=True)
