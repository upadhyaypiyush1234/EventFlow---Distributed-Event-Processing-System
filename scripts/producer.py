    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for i in range(count):
            ...
            if success:
                ...
                if (i + 1) % 10 == 0:
                    print(
                        f"Sent {i + 1}/{count} events (Success: {success_count}, Failed: {failure_count})"
                    )
            else:
                failure_count += 1
...
def main():
    ...
    parser.add_argument(
        "--rate", type=int, default=10, help="Events per second (0 = unlimited)"
    )
    parser.add_argument(
        "--type", choices=EVENT_TYPES, help="Event type (random if not specified)"
    )
